...
// Exporting an asynchronous GET function to handle fetching orders for the customer
export async function GET(req) {
  try {
    ...
    // Fetch the orders added by the current user from the 'orders' collection
    const orders = await db.collection('orders').find({ username: session.username }).toArray();
    await client.close();
...
// Exporting an asynchronous POST function to handle placing orders for the customer
